...
# 撮影ごとの枠付き画像+メタデータ(JSON)を保存するフォルダ名（Photos/ 配下）
AI_HISTORY_DIRNAME = "AI_History"
...
class OpenCVCamera:
    def __init__(
        ...
        self.history_dir = os.path.join(save_dir, AI_HISTORY_DIRNAME)
        ...
        self.last_capture_name = None      # 最新撮影のファイル名（拡張子なし）
        self.last_prediction_id = None     # 最新撮影の model_predictions.id（Save時の更新用）
    ...
    # ============================================================
    # 撮影メタデータ（重量/レベル/ステージ）の保存
    # ============================================================
    def _write_sidecar(self, name):
        """現在の推論結果を AI_History/<name>.json に保存する"""
        cn = self.last_cn or {}
        stage, conf = self.last_stage
        pcm = cn.get("per_class_mass") or {}
        default_lvl = getattr(self.cn_model, "portion_level", "medium")
        levels = {k: self.cn_model.portion_levels.get(k, default_lvl) for k in pcm}
        data = {
            "name": name,
            ...
            "total_mass": cn.get("total_mass"),
            "per_class_mass": pcm,
            "portion_levels": levels,
        }
        path = os.path.join(self.history_dir, f"{name}.json")
    ...
    def save_current_result(self):
        """
        GUIのSaveボタン用: 最新撮影のメタデータを現在のレベル/重量で上書き保存。
        NeonDB の model_predictions 行も更新する（DB失敗でもローカル保存は成功扱い）。
        """
        if not self.last_capture_name:
            return None
        path = self._write_sidecar(self.last_capture_name)

        cn = self.last_cn or {}
        try:
            if self.last_prediction_id:
                self.db.update_prediction_weights(
                ...
                # 撮影時のINSERTが失敗/スキップされていた場合はここで新規作成
                sensor_id = self.db.get_latest_sensor_id(bin_id=BIN_ID)
                if sensor_id:
                    stage, conf = self.last_stage
                    self.last_prediction_id = self.db.insert_model_prediction(
                        bin_id=BIN_ID,
            ...
            print(f"[DB] weight save error: {e}")
    ...
    def _do_capture(self):
        ...
        try:
            ...
            try:
                ...
                # ---------- 履歴保存（枠付き画像 + メタデータJSON） ----------
                self.last_capture_name = f"frame-{ts}"
                try:
                    if cn_ok:
                        shutil.copyfile(
                            ai_path, os.path.join(self.history_dir, f"frame-{ts}.jpg"))
                    self._write_sidecar(self.last_capture_name)
                except Exception as e:
                    print(f"[CAM] history save error: {e}")
