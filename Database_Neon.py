...
想定テーブル:
  ...
  - model_predictions(id, bin_id, sensor_data_id, stage, confidence,
                      cn_ratio, browns_grams, total_mass, created_at)


C/N・推定重量を保存する場合は先に列を追加すること:
  ALTER TABLE model_predictions
    ADD COLUMN IF NOT EXISTS cn_ratio     DOUBLE PRECISION,
    ADD COLUMN IF NOT EXISTS browns_grams DOUBLE PRECISION,
    ADD COLUMN IF NOT EXISTS total_mass   DOUBLE PRECISION;
...
class DatabaseTool:
    ...
    def insert_model_prediction(
        ...
        cn_ratio=None, browns_grams=None, total_mass=None,
    ):
        """
        model_predictions に推論結果を INSERT し、新しい行の id を返す。
        cn_ratio / browns_grams / total_mass 列があればそれも保存。
        無ければ旧4列で保存（idは返す）。
        """
        full_query = """
        INSERT INTO model_predictions (
            bin_id, sensor_data_id, stage, confidence,
            cn_ratio, browns_grams, total_mass
        )
        VALUES (%s,%s,%s,%s,%s,%s,%s)
        RETURNING id;
        """
        full_params = (bin_id, sensor_data_id, stage, confidence,
                       cn_ratio, browns_grams, total_mass)
            ...
            try:
                with conn:
                    with conn.cursor() as cur:
                        ...
                        new_id = cur.fetchone()[0]
                if self.debug:
                    print(f"[DB] model_prediction inserted (+C/N/weight) id={new_id}")
            except psycopg2.errors.UndefinedColumn:
                # cn_ratio/browns_grams/total_mass 列がまだ無い -> 旧4列で再試行
                ...
                with conn:
                    with conn.cursor() as cur:
                        ...
                        new_id = cur.fetchone()[0]
                if self.debug:
                    print(f"[DB] model_prediction inserted (legacy) id={new_id}")
            return new_id
        finally:
            if conn:
                conn.close()
    ...
    # ------------------------------------------------------------
    # Update saved weights on an existing prediction row
    # ------------------------------------------------------------
    def update_prediction_weights(self, prediction_id, cn_ratio=None,
                                  browns_grams=None, total_mass=None):
        """
        Save Estimate ボタン用: レベル調整後の推定値で既存行を上書きする。
        列が無い場合は冒頭docstringの ALTER TABLE を実行すること。
        """
        query = """
        UPDATE model_predictions
           SET cn_ratio = %s, browns_grams = %s, total_mass = %s
         WHERE id = %s;
        """
        conn = None
        try:
            ...
            try:
                with conn:
                    with conn.cursor() as cur:
                        cur.execute(query, (cn_ratio, browns_grams, total_mass, prediction_id))
                        updated = cur.rowcount
                if self.debug:
                    print(f"[DB] prediction id={prediction_id} weights updated ({updated} row)")
                return updated > 0
            except psycopg2.errors.UndefinedColumn:
                print("[DB] cn_ratio/browns_grams/total_mass columns missing — "
                      "run the ALTER TABLE shown at the top of Database_Neon.py")
                return False
        finally:
            if conn:
                conn.close()
